...
final class EluV2URLSessionReplayTransport: EluV2ReplayHTTPTransport, @unchecked Sendable {
    ...
    func send(_ dispatch: EluV2ReplayDispatch) async throws -> EluV1BatchHTTPResponse {
        ...
        defer { use.settle() }
        let request = use.request
        let binding = EluV1TransportAuthority(revalidate: { await use.revalidate() }, isCurrent: { use.beginOnce() })
        ...
        guard EluV1Validation.isAbsoluteHTTPSURI(request.url.absoluteString),
              ...
              request.body.count <= EluV2ReplayPreparedRequest.maximumBytes,
              request.timeoutSeconds.isFinite,
              (1 ... 60).contains(request.timeoutSeconds),
              (1 ... EluV2ReplayResponse.maximumBytes)
        ...
        let operation = EluV2BoundedReplayURLSessionOperation(
            request: urlRequest,
...
private final class EluV2BoundedReplayURLSessionOperation: NSObject,
    ...
    @unchecked Sendable
{
    ...
    func run() async throws -> EluV1BatchHTTPResponse {
        try await withCheckedThrowingContinuation { continuation in
            ...
            let configuration = EluV2ReplaySessionPolicy.configuration(
                timeout: request.timeoutInterval, protocolClasses: protocolClasses
            )
    ...
    private func handleChallenge(
        ...
        // Preserve an observed original refusal even when authentication is
        // declined. No stored, proposed, HTTP, or proxy credential is supplied.
        if let response = challenge.failureResponse as? HTTPURLResponse,
           response.url == request.url, response.statusCode == 401 || response.statusCode == 403 {
            finish(.success(Self.response(response, body: Data())))
        }
        completionHandler(EluV2ReplaySessionPolicy.disposition(challenge.protectionSpace), nil)
    }
    ...
    private func finish(_ result: Result<EluV1BatchHTTPResponse, Error>) {
        lock.lock()
        outcome = EluV2ReplayResponse.preservingRefusal(result, over: outcome)
        guard !completed else { lock.unlock(); return }
        completed = true
        let session = session
        if session == nil {
...
/// A bearer-only HTTP client. Trust evaluation remains the platform default;
/// HTTP/proxy authentication cannot consult ambient credential state.
enum EluV2ReplaySessionPolicy {
    ...
    static func disposition(_ space: URLProtectionSpace) -> URLSession.AuthChallengeDisposition {
        if !space.isProxy(), space.authenticationMethod == NSURLAuthenticationMethodServerTrust {
